...
## 관심사 분리
* 어플리케이션을 하나의 공연이라 생각해보자, 각각의 인터페이스를 배역(배우 역활)이라 생각하자, 그런데! 실제 배역 맞는 배우를 선택하는 것을 누가 하는가?
* 로미오와 줄리엣 공연을 하면 로미오 역활을 누가 할지 줄리엣 역활을 누가 할지는 배우들이 정하는게 아니다.
관심사를 분리하자!
* 배우는 본인의 역화인 배역을 수행하는 것에만 집중해야한다.
* 남자배우는 어떤 여배우가 선택되더라도 똑같이 공연을 할 수 있어야한다.
* 공연을 구성하고, 담당배우는 섭외하고, 역활에 맞는 배우를 지정하는 책임을 담당하는 별도의 공연 기획자 가 나올 시점이다
